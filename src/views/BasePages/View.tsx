      ...
      <div className={"container"}>
        ...
        <Accordion title={"hello"} atomic={atomicState}>
          ...
          <AccordionItem title="Cab Racks" atomic={atomicState}>
            ...
            <p>Yes, Pending approval from a member of our technical staff. For more information about contact Phil Bays
              or James Massey.</p>
            ...
            <p>We only use the highest–strength premium aluminum alloy from 3000 or 5000-series sheet (depending on
              surface finish) 1/8″-thick, corrugated for extra strength and visual appeal.</p>

          </AccordionItem>
          <AccordionItem title="Loading Ramps" atomic={atomicState}>
            <h3>How do Sturdy-Lite Loading Ramps connect to a trailer?</h3>
            <p>Sturdy-Lite provides a bolt-on trailer receiver which mounts over the trailer’s back rub rail, and
              receives the ramp unit’s end adapter in a semi-cylindrical cradle/cusp, which is then pinned into place
              using supplied aluminum rods.</p>


            <h3>What is the highest loading capacity available for your ramp product offerings?</h3>
            <p>Sturdy–Lite’s 16’ Double Ramp Assembly can handle a vehicle which has up to a 20,000 lb load per axle; in
              other words, this ramp assembly can handle up to a 40,000 lb–GVW two–axle vehicle, so long as the
              front-to-rear-axle centerline spacing is at least 96″, and the weight distribution between the front and
              rear axles is no greater than a 3:2 ratio (60%-to-40% margin).</p>


          </AccordionItem>
          <AccordionItem title="Side-Kits" atomic={atomicState}>
            <h3>What roof bow heights are available?</h3>
            <p>Sturdy-Lite roof bows are available in pitches of 10″, 18″, 24″, 28″, 30″, 32″ or 36″ for 96″- and
              102″-wide trailers.</p>
            ...
            <p>Red, white, blue, burgundy, and black are stock color offerings. Other custom colors are available on a
              special-order basis.</p>

            <h3>Do you manufacture side kits for drop-deck trailers?</h3>
            <p>Yes; please call for more information. You will need to measure and record the dimensions requested in
              our side kit survey print–out; see the “Installation” portion of our Side Kits product category webpage to
              download a copy.</p>
          ...
          <AccordionItem title="Toolboxes" atomic={atomicState}>
            ...
            <p>As with all of our products, we use 1/8″-thick aluminum sheet at minimum, corrugated during the
              fabrication process to provide extra strength and load capacity.</p>
          ...
          <AccordionItem title="Underbody Storage Racks (UBRs) for Sidekits" atomic={atomicState}>
            ...
            <p>No, we do not offer stainless doors due to size issues. However, we do offer mirror-finish aluminum
              doors.</p>
          </AccordionItem>
...
export const Services = () => {
  const bulletPoints = [
    "AWS-certified aluminum welding processes",
    "Saw, mill, drill, punch, huck, and self-piercing rivet processes",
    "Hydraulic forming of tube/rod profiles",
    "CNC laser/water-jet operations",
    "CNC press brake and shearing processes",
    "3D CAD design data",
  ];
  ...
  return (
    ...
    <div className={"Services__Section"}>
      <div className="container">
        <h1>Services and Production</h1>

        <p>Sturdy-Lite offers custom aftermarket products which provide loading and storage solutions for commercial
          semi
          trucking. We have an array of equipment for aluminum and steel fabrication, including:</p>

        <ul className={"bullet__list"}>
          {bulletPoints.map(text => <li className={"bullet"}> {text} </li>)}
        </ul>


        <p>If you have an idea for a custom product, please contact Phil or Chad Bays for further discussion.</p>
      </div>
    </div>
